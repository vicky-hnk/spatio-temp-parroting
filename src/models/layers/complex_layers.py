...
import src.models.layers.batchnorm as bn


class UniversalDropout(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.rand(x.shape, device=x.device, dtype=torch.float32)
        keep = (mask > self.p).float() / (1.0 - self.p)

        # For complex: complex_tensor * real_scalar = complex_tensor
        return x * keep
...
class ComplexLayerNorm(nn.Module):
    r"""
    Taken from: https://github.com/torchcvnn/torchcvnn/tree/main
