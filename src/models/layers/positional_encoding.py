"""This file contains Positional Encoding methods for time series data to be used in Transformers."""
...
class TAPE(nn.Module):
    def __init__(self, model_dim:int, max_seq_len:int, dropout=0.1, scale=1.0, classic=False):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
        position = torch.arange(max_seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, model_dim, 2).float() * (-torch.log(torch.tensor(10000.0)) / model_dim))
        angle = position * div_term if classic else (position * div_term) * (model_dim / max_seq_len)
        pe = torch.zeros(max_seq_len, model_dim)
        pe[:, 0::2] = torch.sin(angle)
        pe[:, 1::2] = torch.cos(angle[:, :model_dim // 2])
        # shape [1, 1, seq_len, model_dim]
        self.register_buffer("pe", scale * pe.unsqueeze(0).unsqueeze(0))

    def forward(self, x)-> torch.Tensor:
        # Slice pe, if longer than seq_len
        x = x + self.pe[:, :, :x.size(2), :]
        return self.dropout(x)

class SineSPE(nn.Module):
    """
    See: https://github.com/aliutkus/spe/tree/main
    ...
    def __init__(self, num_heads: int = 8, in_features: int = 64, num_realizations: int = 256, num_sines: int = 1):
        super().__init__()
        ...
        for param in ['freqs', 'offsets', 'gains']:
            self.register_parameter(param, nn.Parameter(torch.randn(num_heads, in_features, num_sines)))
    ...
    def forward(self, shape, num_realizations=None)-> torch.Tensor:
        """
        ...
        Args:
            :arg shape: The outer shape of the inputs: (batchsize, *size)
            :arg num_realizations: if provided, overrides self.num_realizations
        """
        if len(shape) != 2:
            raise ValueError('Only 1D inputs are supported by SineSPE')
        max_len = shape[1]

        # build omega_q and omega_k, with shape (num_heads, keys_dim, length, 2*num_sines)
        ...
        #  sinusoidal basis vectors for each position, head, and feature dimension
        ...
        # gains is (num_heads, keys_dim, num_sines), non-negative with softplus
        ...
        # the number of realizations is overridden by the function argument if provided
        if num_realizations is None:
            num_realizations = self.num_realizations

        # draw noise of appropriate shape on the right device
        z = torch.randn(1, self.num_heads, self.in_features, 2 * self.num_sines,
            num_realizations,
...
class ConvSPE(nn.Module):
    ...
    def __init__(self, ndim: int = 1, num_heads: int = 8, in_features: int = 64, num_realizations: int = 256,
        ...
        self.conv_q = conv_class(in_channels=num_heads * in_features, out_channels=num_heads * in_features,
            stride=1, kernel_size=kernel_size, padding=0, bias=False, groups=num_heads * in_features)

        self.conv_k = conv_class(in_channels=num_heads * in_features, out_channels=num_heads * in_features,
            stride=1, kernel_size=kernel_size, padding=0, bias=False, groups=num_heads * in_features)
...
class SPEFilter(nn.Module):
    ...
    Applies a positional code provided by an SPE module on actual queries and keys.
    ...
    """
    def __init__(self, gated: bool = True, code_shape: Optional[Tuple[int, int]] = None):
        super().__init__()
    ...
    def forward(self, queries: torch.Tensor, keys: torch.Tensor, code: Tuple[torch.Tensor, torch.Tensor]
        ...
        if (len(code_size) != len(query_size)
            or torch.any(torch.tensor(code_size) < torch.tensor(query_size)
            )):
        ...
        # truncate qbar and kbar for matching current queries and keys, if needed
